...
class CEBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        self.pos_embed.to(x.device)
        z0 = z0 + self.pos_embed

        for i in range(self.num_layer):
            z0 = to_4d(self.norm(z0), H, W)
            z0 = self.attn(z0) + z0
...
class SEBlock(nn.Module):
    def __init__(self, dim, patches, num_heads, bias):
        super(SEBlock, self).__init__()
        self.num_layer = 3
        self.norm = nn.LayerNorm(dim * patches**2)
        self.attn = Attention(dim * patches ** 2, num_heads, bias)
        self.patches = patches
        self.pos_embed = nn.Parameter(
            torch.zeros(1, 256, 384 * 2 * 2)
        )
    ...
    def forward(self, x):
        ...
        pos = self.pos_embed.to(x.device).expand(B, -1, -1)
        s = patches + pos

        for i in range(self.num_layer):
            s = to_4d(self.norm(s), N_h, N_w)
            s = self.attn(s) + s
            s = to_3d(s)

        s = s.reshape(B, N_h, N_w, C, P, P) \
             .permute(0, 3, 1, 4, 2, 5) \
             .reshape(B, C, H, W)
        return s + x
...
class TransformerBlock(nn.Module):
    def __init__(self, dim, num_heads, ffn_expansion_factor, bias, LayerNorm_type):
        ...
        if (dim == 384):
            self.ceblock = CEBlock(dim, num_heads, bias)
            self.seblock = SEBlock(dim, 2, num_heads, bias)
...
class RefineBlock(nn.Module):
    def __init__(self, inp_channel=3):
        ...
        self.refine = nn.Sequential(
            nn.Conv2d(inp_channel, 32, 3, 1, 1),
            nn.ReLU(),
            nn.Conv2d(32, inp_channel, 1, 1)
        )
...
class PromptIR(nn.Module):
    ...
    def forward(self, inp_img, noise_emb=None):
        ...
        output = self.final_refine(out_dec_level1)
        return output
